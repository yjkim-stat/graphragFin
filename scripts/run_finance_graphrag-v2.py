...
def _dataframe_to_records(df: Any) -> list[dict[str, Any]]:
    """Convert a pandas DataFrame into JSON-serialisable records."""

    if df is None:
        return []
    if not hasattr(df, "replace") or not hasattr(df, "to_dict"):
        return []
...
async def _run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    text_units_df = artifacts.get("text_units")
    if text_units_df is not None and "document_ids" in text_units_df.columns:
        artifacts["text_units"] = text_units_df.assign(
            document_ids=text_units_df["document_ids"].astype(str)
        )
    ...
    if args.skip_community_reports:
        required_artifacts.remove("community_reports")
    ...
    extracted_entities_by_document = _entities_by_document(
        artifacts["entities"],
        artifacts["text_units"],
    )
    retrieved_entity_names = {
        str(record.get("entity")).strip()
        for record in context_records.get("entities", [])
        if isinstance(record.get("entity"), str)
        and str(record.get("entity")).strip()
    }
    retrieved_entities_by_document = {
        doc_id: {
            entity
            for entity in entity_set
            if entity in retrieved_entity_names
        }
        for doc_id, entity_set in extracted_entities_by_document.items()
    }
    retrieved_entities_by_document = {
        doc_id: values
        for doc_id, values in retrieved_entities_by_document.items()
        if values
    }
    retrieved_vertex_metrics = _evaluate_ner(
        ground_truth_entities,
        retrieved_entities_by_document,
    )
    retrieved_vertices_sorted = sorted(
        retrieved_entity_names, key=lambda item: item.casefold()
    )

    export_dir = root_dir / "exports"
    export_dir.mkdir(parents=True, exist_ok=True)

    response_payload = search_result.response
    ...
    graph_export_path = export_dir / "graph_data.json"
    graph_export_payload = {
        key: _dataframe_to_records(artifacts.get(key))
        for key in ["entities", "relationships", "communities"]
    }
    ...
    community_export_path = export_dir / "community_summaries.json"
    community_export_payload = {
        "retrieved_reports": context_records.get("reports", []),
        "community_reports": _dataframe_to_records(
            artifacts.get("community_reports")
        ),
    }
    community_export_path.write_text(
        json.dumps(
            community_export_payload,
            ensure_ascii=False,
            indent=2,
    ...
    local_search_export_path = export_dir / "local_search_output.json"
    local_search_export_payload = {
        "query": args.query,
        ...
        "retrieved_vertices": retrieved_vertices_sorted,
        "retrieved_vertex_metrics": retrieved_vertex_metrics,
        "graph_summary": graph_info,
        "pipeline_stats": stats_data,
        "pipeline_state": context_state,
        "workflows": workflow_summaries,
        "dataset": dataset_summary,
    }
    local_search_export_path.write_text(
        json.dumps(
            local_search_export_payload,
            ensure_ascii=False,
            indent=2,
    ...
    report = {
        ...
        "export_paths": {
            "graph_data": str(graph_export_path.resolve()),
            "community_summaries": str(community_export_path.resolve()),
            "local_search_output": str(local_search_export_path.resolve()),
        },
    }
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "When provided, retrieved vertices are evaluated against these annotations."
        ),
    )
    parser.add_argument(
        "--model-name",
