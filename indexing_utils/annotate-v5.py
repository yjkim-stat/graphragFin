...
def remove_bracket_suffix(text):
    if text.endswith("]"):
        idx = text.rfind("[")
        if idx != -1:
...
# df[args.text_column] = df[args.text_column].apply(lambda text: re.sub(r"\[\+\d+\schars\]", "", text))

df[args.text_column] = df[args.text_column].apply(remove_bracket_suffix)
df[args.text_column] = df[args.text_column].str.replace('…', '').str.strip(' ')
